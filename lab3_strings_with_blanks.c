...
char *my_strcpy(char *dst, char *src) {
    ...
    int i = 0;
    // Loop through each character of the source array and copy it to the destination array
    while(src[i] != '\0'){
        dst[i] = src[i];
        i++;
    }
    
    // Add a null terminator at the end of the destination array
    dst[i] = '\0';
