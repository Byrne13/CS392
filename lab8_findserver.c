...
int main() {
    ...
    for (port = 1024; port <= 65535; port++) {
        ...
        //If it does connect, Attempt to receive up to BUFLEN - 1 characters.
        if ((bytes_recvd = recv(client_socket, buf, BUFLEN - 1, 0)) == -1) {
        ...
        //If data is received, Null-terminate it.
        buf[bytes_recvd] = '\0';
